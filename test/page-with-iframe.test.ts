...
beforeAll(async () => {
  ...
  await page.goto(
    `file://${path.join(__dirname, 'fixtures/page-with-iframe.html')}`,
  )

  frame = await (await page.$('iframe[title="Main Content"]')).contentFrame()
})
...
test('should wait for element to become visible before testing', async () => {
  await frame.evaluate(elementToHide => {
